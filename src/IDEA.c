...
// devuelve un numero x1 tal que (z1*x1) mod (2^16 + 1) == 1. Se usa el algorimo de euclides para
// ello
char* mul_modular_inversa(char* z1) {
    ...
    if (t < 0) t += 65537;  // sumo 2^16 + 1 para que se vuelva positivo
...
char** generar_subclaves(char* clave) {
    ...
    for (int i = 0; clave[i] != '\0'; i = i + 4) {
        char* subclave_n =
            string_from_format("%c%c%c%c", clave[i], clave[i + 1], clave[i + 2], clave[i + 3]);
    ...
    void append_array(char* subclave_en_ascii) { string_append(&clave_ascii, subclave_en_ascii); }
    ...
    for (int i = 52; i < 56; i++) { free(subclaves_ascii[i]); }
...
char** generar_subclaves_desencriptar(char* clave) {
    ...
    void buscar_null(char* subclave_n) {
        if (string_contains(subclave_n, "NULL")) { clave_invalida = 1; }
    }
    ...
    // subclaves ronda 1
    string_array_push(
        &subclaves_array_desencriptar,
        mul_modular_inversa(subclaves_array[48]));  // 48 = 6* 8 es decir, k1 de la ronda 9

    string_array_push(
        &subclaves_array_desencriptar,
        suma_modular_inversa(subclaves_array[49]));  // 49 = 6* 8 + 1 es decir, k2 de la ronda 9

    string_array_push(
        &subclaves_array_desencriptar,
        suma_modular_inversa(subclaves_array[50]));  // 50 = 6* 8 + 2 es decir, k3 de la ronda 9

    string_array_push(
        &subclaves_array_desencriptar,
        mul_modular_inversa(subclaves_array[51]));  // 51 = 6* 8 + 3 es decir, k4 de la ronda 9

    string_array_push(&subclaves_array_desencriptar,
                      strdup(subclaves_array[46]));  // 46 = 6* 7 + 4 es decir, k5 de la ronda 8

    string_array_push(&subclaves_array_desencriptar,
                      strdup(subclaves_array[47]));  // 47 = 6* 7 + 5 es decir, k6 de la ronda 8

    // subclaves de la ronda 2 a la 8
    for (int ronda = 1; ronda < 8; ronda++) {
        string_array_push(&subclaves_array_desencriptar,
                          mul_modular_inversa(
                              subclaves_array[6 * (10 - ronda - 2)]));  // k1 de la ronda (10-ronda)

        string_array_push(
            &subclaves_array_desencriptar,
            suma_modular_inversa(
                subclaves_array[6 * (10 - ronda - 2) + 2]));  // k3 de la ronda (10-ronda)

        string_array_push(
            &subclaves_array_desencriptar,
            suma_modular_inversa(
                subclaves_array[6 * (10 - ronda - 2) + 1]));  // k2 de la ronda (10-ronda)

        string_array_push(
            &subclaves_array_desencriptar,
            mul_modular_inversa(
                subclaves_array[6 * (10 - ronda - 2) + 3]));  // k4 de la ronda (10-ronda)

        string_array_push(
            &subclaves_array_desencriptar,
            strdup(subclaves_array[6 * (9 - ronda - 2) + 4]));  // k5 de la ronda (9-ronda)

        string_array_push(
            &subclaves_array_desencriptar,
            strdup(subclaves_array[6 * (9 - ronda - 2) + 5]));  // k6 de la ronda (9-ronda)
    }
    // subclaves ronda 9

    string_array_push(
        &subclaves_array_desencriptar,
        mul_modular_inversa(subclaves_array[0]));  // 0 = 6* 0  es decir, k1 de la ronda 1

    string_array_push(
        &subclaves_array_desencriptar,
        suma_modular_inversa(subclaves_array[1]));  // 1 = 6* 0 + 1 es decir, k2 de la ronda 1

    string_array_push(
        &subclaves_array_desencriptar,
        suma_modular_inversa(subclaves_array[2]));  // 2 = 6* 0 + 2 es decir, k3 de la ronda 1

    string_array_push(
        &subclaves_array_desencriptar,
        mul_modular_inversa(subclaves_array[3]));  // 3 = 6* 0+3  es decir, k4 de la ronda 1
...
void bits_a_rellenar(uint32_t numero_relleno, int cant_disponible, int bits_tam_total,
                     char** buffer) {
    ...
    for (int i = 0; i < cant_a_repetir; i++) { strcat(*buffer, bits_a_multiplicar); }
...
void sacar_bytes_relleno(char** descifrado_completo) {
    ...
    } else if (nums[7] <= 7) {  // Nunca puede ser 8 sino es una coincidencia ya que un bloque tiene
                                // 8 bytes en total
        int count = 0;
        for (int i = 7; nums[i] == nums[7];
             i--) {  // calculo la cantidad de veces que se repite el mismo numero en el bloque
            count++;
        }
        if (count == nums[7]) {  // sino solo es una coincidencia
            char* temp =
                string_substring_until(*descifrado_completo, tam_descifrado - (nums[7] * 8));
            free(*descifrado_completo);
            *descifrado_completo = temp;
        }
    }
...
char** obtener_bloques(char* contenido, int es_cifrado, int len_contenido) {
    ...
    if (tam_contenido_bits % 64 != 0) { cant_bloques++; }
...
char* mul_modular(char* x1, char* x2) {
    ...
    if (resultado_num == 65536) { resultado_num = 0; }
...
char** media_ronda(char** subbloques_n, char** subclaves) {
    ...
    string_array_push(
        &subbloque_cifrado,
        mul_modular(subbloques_n[0], subclaves[48]));  // 6*8 = 48 que es k1 de la ronda 9

    string_array_push(
        &subbloque_cifrado,
        suma_modular(subbloques_n[2], subclaves[49]));  // 6*8+1 = 49 que es k2 de la ronda 9

    string_array_push(&subbloque_cifrado,
                      suma_modular(subbloques_n[1], subclaves[50]));  // 6*8+2 = 50 k3 de la ronda 9

    string_array_push(
        &subbloque_cifrado,
        mul_modular(subbloques_n[3], subclaves[51]));  // 6*8+3 = 51 que es k4 de la ronda 9
...
int main(int argc, char** argv) {
    if (argc < 5) {
        printf(
            "Se debe indicar la clave, el nombre del archivo txt, si se quiere encriptar (1) o "
            "desencriptar(0) y la ruta al archivo con el resultado (debe ser txt) !!\n");
        return -1;
    }
    ...
    for (int i = 0; i < 52; i++) {
        printf("subclave numero %i (en total %i bits): %s\n", i, string_size(subclaves[i]),
               subclaves[i]);
    }

    char** bloques = obtener_bloques(
        contenido, es_cifrado, stat_file.st_size);  // separo el contenido en bloques de 64 bits
    ...
    for (int i = 0; i < cant_bloques; i++) {
        ...
        char** subbloques_n =
            obtener_subbloques(bloques[i]);  // separo el bloque en 4 subbloques de 16 bits

        for (int k = 0; k < 4; k++) { printf("\tsubbloque %i: %s\n", k, subbloques_n[k]); }

        // aplico las 8 rondas
        for (int j = 0; j < round; j++) { ronda(subbloques_n, j, subclaves); }
    ...
    if (!es_cifrado) { sacar_bytes_relleno(&cifrado_completo); }
